(function () {
	...
	$.extend(erpnext, {
		...
		route_to_pending_reposts: function (args) {
	...
	erpnext.utils.update_child_items = function(opts) {
		...
		this.data = frm.doc[opts.child_docname].map(function (d) {
			return {
				"docname": d.name,
				"name": d.name,
	...
	*/
	...
	erpnext.taxes_and_totals = erpnext.payments.extend({
		...
		calculate_item_values: function() {
			...
			if (!this.discount_amount_applied) {
				for (var i = 0, list = this.frm.doc.items || []; i < list.length; i += 1) {
					var item = list[i];
					...
					if (!(me.frm.doc.is_return || me.frm.doc.is_debit_note)) {
						...
						var qty = item.qty || (me.frm.doc.is_debit_note ? 1 : -1);
	...
	erpnext.TransactionController = erpnext.taxes_and_totals.extend({
		...
		is_a_mapped_document: function is_a_mapped_document(item) {
			var mapped_item_field_map = {
			...
			var mappped_fields = mapped_item_field_map[item.doctype] || [];
		...
		_set_values_for_item_list: function(children) {
			...
			for(var i=0, l=children.length; i<l; i++) {
				var d = children[i] ;
				var item_row = frappe.get_doc(d.doctype, d.name);
		...
		make_payment_entry: function make_payment_entry() {
			var via_journal_entry = this.frm.doc.__onload && this.frm.doc.__onload.make_payment_via_journal_entry;
		...
		make_mapped_payment_entry: function make_mapped_payment_entry(args) {
			var me = this;
		...
		prompt_user_for_reference_date: function prompt_user_for_reference_date(){
			var me = this;
			frappe.prompt({
		...
		has_discount_in_schedule: function has_discount_in_schedule() {
			var is_eligible = in_list(
				["Sales Order", "Sales Invoice", "Purchase Order", "Purchase Invoice"],
				this.frm.doctype
			);
			var has_payment_schedule = this.frm.doc.payment_schedule && this.frm.doc.payment_schedule.length;
			if(!is_eligible || !has_payment_schedule) { return false; }

			var has_discount = this.frm.doc.payment_schedule.some(function (row) { return row.discount_date; });
			return has_discount;
		},

		make_quality_inspection: function make_quality_inspection() {
			var data = [];
			var fields = [
				{
					label: "Items",
		...
		coupon_code: function() {
			...
			if (this.frm.doc.coupon_code || this.frm._last_coupon_code) {
				...
				var _ignore_pricing_rule = this.frm.doc.ignore_pricing_rule;
				return frappe.run_serially([
					function () { return this$1.frm.doc.ignore_pricing_rule=1; },
					function () { return this$1.frm.trigger('ignore_pricing_rule'); },
					function () { return this$1.frm.doc.ignore_pricing_rule=_ignore_pricing_rule; },
					function () { return this$1.frm.trigger('apply_pricing_rule'); },
					function () { return this$1.frm._last_coupon_code = this$1.frm.doc.coupon_code; }
				]);
			}
		}
	});
			...
			this.call_details = new frappe.ui.FieldGroup({
				fields: [{
					...
					'click': function () {
						...
						var call_type = this$1.call_details.get_value('call_type');
